...
$bankStmt = $pdo->prepare(
    'SELECT apm.bank_name, apm.account_holder, apm.account_number, apm.account_type,
            apm.currency, apm.alias_or_reference, apm.payment_email, apm.is_active
     FROM associate_payment_methods apm
     INNER JOIN associate_offers ao ON ao.user_id = apm.user_id
     WHERE ao.referral_code = :code AND apm.is_active = 1
     LIMIT 1'
);
$bankStmt->execute(['code' => $code]);
$bankRow = $bankStmt->fetch(PDO::FETCH_ASSOC);
if (is_array($bankRow)) {
    $bankDetails = [
        'bankName' => (string)($bankRow['bank_name'] ?? ''),
        'accountHolder' => (string)($bankRow['account_holder'] ?? ''),
        'accountNumber' => (string)($bankRow['account_number'] ?? ''),
        'accountType' => (string)($bankRow['account_type'] ?? ''),
        'currency' => (string)($bankRow['currency'] ?? ''),
        'aliasOrReference' => (string)($bankRow['alias_or_reference'] ?? ''),
        'paymentEmail' => (string)($bankRow['payment_email'] ?? ''),
    ];
}
