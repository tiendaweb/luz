(() => {
  ...
  async function loadAdminPages() {
    ...
    try {
      const result = await window.appApiFetch("/api/admin/pages/list.php");
          ...
          <article class="rounded-xl border border-slate-200 bg-slate-50 p-3">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p class="text-xs text-slate-500">Ruta: <a href="${viewUrl}" target="_blank" class="underline text-emerald-700">${escapeHtml(viewUrl)}</a></p>
              </div>
      ...
      list.querySelectorAll('[data-action="edit-page"]').forEach((button) => {
        button.addEventListener("click", async () => {
          ...
          try {
            const result = await window.appApiFetch(`/api/admin/pages/show.php?id=${encodeURIComponent(id || "")}`);
      ...
      list.querySelectorAll('[data-action="delete-page"]').forEach((button) => {
        button.addEventListener("click", async () => {
          ...
          try {
            await window.appApiFetch(`/api/admin/pages/delete.php?id=${encodeURIComponent(id || "")}`, { method: "DELETE" });
  ...
  function wireEvents() {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const endpoint = isEdit ? "/api/admin/pages/update.php" : "/api/admin/pages/create.php";
