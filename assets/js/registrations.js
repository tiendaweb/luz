(() => {
      ...
      if (adminCards) {
        adminCards.innerHTML = `
          ...
          <div class="bg-white rounded-3xl p-8 shadow-lg border border-slate-100">
            ...
            <h3 class="text-3xl font-extrabold text-amber-700 mb-4">${summary.users_total ?? 0}</h3>
  ...
  function renderAssociateRegistrations(items) {
    ...
    target.innerHTML = items.map((item) => `
      <article class="rounded-xl border border-amber-100 bg-white p-3">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          ...
          <div class="flex items-center gap-2">
            <button data-action="associate-approve" data-id="${item.id}" class="rounded-lg bg-amber-600 px-2 py-1 text-xs font-bold text-white">Aprobar</button>
  ...
  function renderAdminUsers(items) {
    ...
    target.innerHTML = filtered.map((item) => {
      const explicitBadge = item.has_explicit_flags
        ? '<span class="rounded-full bg-amber-100 px-2 py-1 text-[11px] font-bold text-amber-700">Flags explícitos</span>'
  ...
  function renderAssociateNetwork(items, referralLink) {
    ...
    overview.innerHTML = `
      ...
      <div class="bg-white rounded-3xl p-8 shadow-lg border border-slate-100">
        ...
        <h3 class="text-3xl font-extrabold text-amber-600 mb-4">${approved}</h3>
          ...
          <article class="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                ${item.payment_link ? `<a class="text-xs text-amber-800 underline" href="${item.payment_link}" target="_blank" rel="noopener noreferrer">Link de pago esperado</a>` : ""}
              </div>
              <div class="space-y-2 md:text-right">
                ...
                <div class="flex gap-2 md:justify-end">
                  <button data-action="associate-approve" data-id="${item.id}" class="rounded-lg bg-amber-600 px-3 py-1 text-xs font-bold text-white">Aprobar</button>
  ...
  function renderUserPaymentStatus(items) {
    ...
    target.innerHTML = `
      ...
      <div class="bg-white rounded-3xl p-8 shadow-lg border border-slate-100">
        ...
        <h3 class="text-2xl font-extrabold text-amber-700 mb-4">${originReferral}</h3>
      ...
      <div class="bg-white rounded-3xl p-8 shadow-lg border border-slate-100">
        ...
        <div class="mt-4 w-full bg-slate-200 rounded-full h-2 mb-3">
          <div class="bg-amber-700 h-2 rounded-full" style="width: ${Math.max(0, Math.min(progress, 100))}%"></div>
        </div>
  ...
  function renderUserEbooks(items) {
    ...
    target.innerHTML = items.map((item) => {
      ...
      const badge = canDownload
        ? '<span class="rounded-full bg-amber-100 px-2 py-1 text-[11px] font-bold text-amber-700">Acceso habilitado</span>'
        : '<span class="rounded-full bg-rose-100 px-2 py-1 text-[11px] font-bold text-rose-700">Sin acceso</span>';
      const action = canDownload
        ? `<a href="${item.download_url}" class="rounded-lg bg-amber-700 px-3 py-2 text-xs font-bold text-white hover:bg-amber-800">Descargar</a>`
        : '<span class="text-[11px] font-semibold text-slate-500">Cumple aprobación o asistencia mínima para habilitar.</span>';

      return `
      <article class="rounded-xl border border-amber-100 bg-amber-50/40 p-3">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
      ...
      return `
        <article class="rounded-2xl border border-amber-200 bg-white p-4">
          ...
          <ul class="mt-2 text-xs space-y-1">
            <li class="${ebooks ? "text-amber-700" : "text-slate-500"}">${ebooks ? "✅" : "🔒"} eBooks ${ebooks ? "habilitados" : "bloqueados (requiere aprobación)"}</li>
            <li class="${cert ? "text-amber-700" : "text-slate-500"}">${cert ? "✅" : "🔒"} Certificado ${cert ? "habilitado" : "bloqueado (requiere aprobación + 75% asistencia)"}</li>
          </ul>
        </article>
      `;
    }).join("");
  ...
  function setupRegistrationForm() {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const email = emailField ? String(formData.get("email") || "").trim() : "";
      const password = passwordField ? String(formData.get("password") || "").trim() : "";
      const fullName = String(formData.get("fullName") || "").trim();
      const documentId = String(formData.get("documentId") || "").trim();
      const forumId = Number(formData.get("forumId") || 0);

      // Only validate email/password when fields exist (standalone inscription page)
      if (emailField) {
        if (!email) {
          showRegisterFeedback("error", "El email es obligatorio.");
          return;
        }
        if (!password || password.length < 6) {
          showRegisterFeedback("error", "La contraseña debe tener al menos 6 caracteres.");
          return;
        }
      }

      if (!fullName) {
        showRegisterFeedback("error", "El nombre y apellidos son obligatorios.");
        return;
      }
      if (!documentId) {
        showRegisterFeedback("error", "El número de documento es obligatorio.");
        return;
      }
      if (forumId <= 0) {
        showRegisterFeedback("error", "Debes seleccionar un foro.");
        return;
      }
      if (!accepted) {
      ...
      const originalBtnText = submitBtn?.textContent || "Confirmar Inscripción";
      ...
      try {
        ...
        if (proofFile instanceof File && proofFile.size > 0) {
          showRegisterFeedback("success", "Procesando comprobante de pago...");
        ...
        showRegisterFeedback("success", "Guardando inscripción...");

        const registrationResult = await window.appApiFetch("/api/registrations/create", {
          method: "POST",
          body: JSON.stringify({
        ...
        showRegisterFeedback("success", "✓ Inscripción registrada con éxito. Iniciando sesión...");

        // Auto-login if user was just created with email/password
        if (emailField && email && password) {
          try {
            await window.appApiFetch("/api/auth/login", {
              method: "POST",
              body: JSON.stringify({ email, password })
            });
            showRegisterFeedback("success", "✓ Sesión iniciada. Redirigiendo a tu cuenta...");
          } catch (loginError) {
            console.warn("Auto-login failed, user can log in manually", loginError);
            showRegisterFeedback("success", "Inscripción completada. Por favor inicia sesión manualmente.");
          }
        }
        ...
        showRegisterFeedback("error", `✗ Error al guardar: ${errorMsg}. Intenta nuevamente.`);
      } finally {
        if (submitBtn) {
  ...
  async function loadForumOptions() {
    const selects = document.querySelectorAll("select#forumIdSelect");
    if (selects.length === 0) return;
    try {
      // /api/forums/list returns { items: [...] } — see public/api/forums/list.php.
      const result = await window.appApiFetch("/api/forums/list?per_page=20");
      const forums = Array.isArray(result.items)
        ? result.items
        : (Array.isArray(result.forums) ? result.forums : []);
      const placeholder = '<option value="">Selecciona tu grupo y horario...</option>';
      if (forums.length === 0) {
        selects.forEach(s => { s.innerHTML = '<option value="">No hay foros disponibles</option>'; });
        return;
      }
      const optionsHtml = placeholder + forums.map(f => {
        const label = f.title || f.code || `Foro #${f.id}`;
        return `<option value="${f.id}">${label}</option>`;
      }).join("");
      selects.forEach(s => { s.innerHTML = optionsHtml; });
    } catch (_err) {
      selects.forEach(s => { s.innerHTML = '<option value="">Error al cargar foros — recarga la página</option>'; });
    }
  }

  window.addEventListener("DOMContentLoaded", async () => {
    setupRegistrationCountrySelector();
    await loadForumOptions();
